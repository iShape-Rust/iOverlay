...
#[derive(Clone)]
pub struct ScanSegment<Id: Copy> {
    pub id: Id,
    pub range: LineRange,
    pub stop: i64,
}
...
pub struct ScanSpace<Id: Copy> {
    pub indexer: LineIndexer,
    heaps: Vec<Vec<ScanSegment<Id>>>,
    index_buffer: Vec<usize>,
}

impl<Id: Copy> ScanSpace<Id> {
    pub(crate) fn new(range: LineRange, count: usize) -> Self {
        ...
        let heaps: Vec<Vec<ScanSegment<Id>>> = vec![Vec::new(); indexer.size];

        Self { indexer, heaps, index_buffer: Vec::new() }
    }

    pub(crate) fn insert(&mut self, segment: ScanSegment<Id>) {
    ...
    pub fn items_in_range(&mut self, range: LineRange, stop: i64, items: &mut Vec<ScanItem<Id>>) {
    ...
    pub fn ids_in_range(&mut self, range: LineRange, stop: i64, ids: &mut Vec<Id>) {
        self.indexer.fill_unsafe(range, &mut self.index_buffer);
