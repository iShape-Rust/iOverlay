...
impl EditorApp {
    pub(crate) fn variable_stroke_workspace(&mut self, ui: &mut egui::Ui) {
        self.update(AppMessage::VariableStroke(
            VariableStrokeMessage::WorkspaceSized(ui.available_size()),
        ));
        let workspace = &mut self.state.variable_stroke.workspace;
        ...
        if let Some(update) = update {
            self.update(AppMessage::VariableStroke(
                VariableStrokeMessage::PointEdited(update),
            ));
            ui.ctx().request_repaint();
        }
        let workspace = &self.state.variable_stroke.workspace;
    ...
    pub(super) fn variable_stroke_update_point(
        &mut self,
