...
impl OutlineTest {
    fn load(index: usize, folder: &str) -> Option<Self> {
        ...
        let result: Result<OutlineTest, _> = serde_json::from_str(&data);
        match result {
            Ok(test) => Some(test),
            Err(e) => {
...
impl OutlineResource {
    ...
    pub(crate) fn with_content(content: &String) -> Self {
        let tests_vec: Vec<OutlineTest> = serde_json::from_str(content).unwrap_or_else(|e| {
            eprintln!("Failed to parse JSON content: {}", e);
            vec![]
        });

        let tests: HashMap<usize, OutlineTest> = tests_vec
            .into_iter()
    ...
    pub(crate) fn load(&mut self, index: usize) -> Option<OutlineTest> {
        if self.count <= index {
            return None;
        }
