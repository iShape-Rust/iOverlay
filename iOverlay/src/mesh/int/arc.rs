//! Directed unit-circle arcs using cached integer rotation matrices.
...
/// Settings shared by all arcs built with an [`ArcBuilder`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ArcOptions {
    ...
    /// This changes matrix construction cost, not the coefficient format.
...
impl<I: IntNumber> ArcBuilder<I> {
    ...
    pub fn new(options: ArcOptions) -> Self {
        let options = ArcOptions {
            ...
            rotation_precision: options.rotation_precision.clamp(4, 32),
        };
...
#[cfg(test)]
mod tests {
    use super::{ArcBuilder, ArcDirection, ArcOptions};
    use i_float::int::vector::IntVector;
    ...
    fn repeated_builds_reuse_buffer_and_rotations() {
        let from = IntVector::<i32>::new(1, 0).fast_normalize().unwrap();
        let to = IntVector::<i32>::new(0, -1).fast_normalize().unwrap();
        let mut builder = ArcBuilder::new(ArcOptions {
